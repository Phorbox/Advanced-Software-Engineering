...
<body onload="menuFunction()">
  ...
  <table hidden id="pagination">
    <tr>
      <td><button onclick="displayResults()" value=0 id="Start" hidden>Start</button></td>
      <td><button onclick="displayResults()" value=0 id="Previous" hidden>Previous</button></td>
      <td>
        <div value=0 id="current">current</div>
      </td>
      <td><button onclick="displayResults()" value=0 id="Next">Next</button></td>
    </tr>
  </table>
  ...
  <script type="text/javascript">
    ...
    async function displayResults() {
      ...
      let query = `http://localhost/api/results?brand=${brand}&type=${type}&serial=${serial}&offset=${offset}`;
      ...
      if (jsonData['data']['hasNext']) {
        document.getElementById("pagination").setAttribute("hidden", false);
      }
